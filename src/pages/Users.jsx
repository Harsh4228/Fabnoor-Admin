...
const Users = ({ token }) => {
  ...
  const [sortType, setSortType] = useState("latest"); // latest, name
  ...
  const handleUserClick = async (user) => {
    ...
    try {
      const res = await axios.get(
        `${backendUrl}/api/user/admin/user/${user._id}`,
        {
          headers: { Authorization: `Bearer ${token}` },
        },
      );
  ...
  const handleMakeAdmin = async (userId, userName) => {
    if (
      !window.confirm(
        `Are you sure you want to promote ${userName} to Admin? They will have full access to this panel.`,
      )
    ) {
      return;
    }
    ...
    try {
      const res = await axios.post(
        `${backendUrl}/api/user/admin/make-admin`,
        { id: userId },
        { headers: { Authorization: `Bearer ${token}` } },
      );
      ...
      toast.error(
        err.response?.data?.message || "Error promoting user to admin",
      );
    } finally {
  ...
  const handleRemoveAdmin = async (userId, userName) => {
    ...
    if (
      !window.confirm(
        `Are you sure you want to revoke Admin access for ${userName}? They will lose all access to this panel.`,
      )
    ) {
      return;
    }
    ...
    try {
      const res = await axios.post(
        `${backendUrl}/api/user/admin/remove-admin`,
        { id: userId },
        { headers: { Authorization: `Bearer ${token}` } },
      );
      ...
      <div className="flex flex-col md:flex-row md:items-end justify-between gap-4 mb-6">
        <div>
          ...
          <p className="text-sm text-slate-500 mt-1">
            Manage customers, shop profiles, and administrative access
          </p>
        </div>
        ...
        <div className="flex flex-col sm:flex-row gap-3">
          ...
          <div className="relative w-full sm:w-80 group">
            ...
            <input
              ...
              className="w-full pl-10 pr-8 py-2.5 bg-white border border-slate-200 rounded-lg shadow-sm outline-none focus:border-blue-500 focus:ring-4 focus:ring-blue-500/10 transition-all text-sm"
            />
            {searchTerm && (
              <button
          ...
          <button
            onClick={() =>
              setSortType((prev) => (prev === "latest" ? "name" : "latest"))
            }
            className={`flex items-center justify-center gap-2 border px-4 py-2 rounded-lg font-semibold shadow-sm transition-all text-sm ${sortType === "name" ? "bg-indigo-50 border-indigo-200 text-indigo-600" : "bg-white border-slate-200 text-slate-600 hover:border-blue-300"}`}
          >
            <svg
              className="w-4 h-4"
              fill="none"
            ...
            {sortType === "latest" ? "Sort: Latest" : "Sort: A-Z"}
          </button>

          <button
            onClick={() => setIsStaffDrawerOpen(true)}
            className="flex items-center justify-center gap-2 bg-slate-900 hover:bg-slate-800 text-white px-4 py-2 rounded-lg font-semibold shadow-lg transition-all text-sm border border-slate-800"
          >
            <svg
              className="w-4 h-4"
              fill="none"
                      ...
                      >
                        ...
                        <td className="px-6 py-4 text-center">
                          <div className="flex flex-col items-center gap-1.5">
                            ...
                            <span
                              className={`px-2 py-0.5 rounded text-[10px] font-bold uppercase tracking-tight ${u.role === "admin" ? "bg-blue-100 text-blue-700 border border-blue-200" : "bg-slate-100 text-slate-500 border border-slate-200"}`}
                            >
                              {u.role}
                            </span>
                          </div>
                          ...
                          <div className="flex items-center justify-center gap-2">
                            {u.role === "admin" ? (
                              ...
                              <button
                                ...
                                className="px-3 py-1 bg-white hover:bg-slate-50 border border-slate-200 text-slate-600 rounded-lg text-[10px] font-bold transition-all"
                              >
                                DETAILS
                              </button>
                            )}
                  ...
                  <div className="flex items-center gap-4">
                    ...
                    <div>
                      <div className="flex items-center gap-2 mb-1">
                        <span className="px-2 py-0.5 bg-blue-500 text-[10px] font-black uppercase tracking-widest rounded">
                          User Profile
                        </span>
                ...
                {userDetails && (
                  <div className="flex overflow-x-auto gap-4 mt-8 -mb-1 relative z-10 no-scrollbar">
                    {[
                      ...
                      {
                        id: "orders",
                        label: `ORDERS (${userDetails.orders?.length || 0})`,
                      },
                      {
                        id: "cart",
                        label: `CART (${Object.keys(userDetails.user?.cartData || {}).length})`,
                      },
                      {
                        id: "wishlist",
                        label: `WISHLIST (${userDetails.user?.wishlist?.length || 0})`,
                      },
                    ].map((tab) => (
                      <button
                        key={tab.id}
                        onClick={() => setActiveTab(tab.id)}
                        className={`px-2 py-3 font-black text-[10px] tracking-widest transition-all whitespace-nowrap border-b-2 ${
                          activeTab === tab.id
              ...
              <div className="p-8 overflow-y-auto flex-1 bg-slate-50/50 scrollbar-thin">
                {detailsLoading ? (
                  ...
                  <div className="animate-in fade-in duration-300">
                        ...
                        <div className="space-y-8">
                          <div className="bg-white p-6 rounded-2xl shadow-sm border border-slate-200">
                            ...
                            <div className="space-y-6">
                                ...
                                <div>
                                  ...
                                  <div className="flex items-center gap-2">
                                    ...
                                    {userDetails.user.role === "admin" && (
                                      <span className="bg-blue-100 text-blue-700 px-2 py-0.5 rounded text-[9px] font-black uppercase tracking-tighter shadow-sm border border-blue-200">
                                        Internal Staff
                                      </span>
                                    )}
                                  </div>
                              ...
                              <div>
                                ...
                                <p className="font-bold text-slate-800">
                                  {userDetails.user.dob || "Not registered"}
                                </p>
                              </div>
                            </div>
                          </div>
                          ...
                          <div className="bg-white p-6 rounded-2xl shadow-sm border border-slate-200">
                            <div className="flex items-center gap-2 mb-6 border-b border-slate-100 pb-4">
                              ...
                              <h3 className="font-bold text-slate-800 uppercase tracking-wider text-xs">
                                Saved Logistics Entry
                              </h3>
                            </div>
                            {userDetails.user.address?.street ? (
                              <div className="space-y-1.5 text-slate-700 text-xs font-medium leading-relaxed">
                                <p className="font-bold text-slate-900 border-b border-slate-50 pb-2 mb-2">
                                  {userDetails.user.address.street}
                                </p>
                                <p>
                                  {userDetails.user.address.city},{" "}
                                  {userDetails.user.address.state}
                                </p>
                        ...
                        <div className="space-y-8">
                            ...
                            {userDetails.user.role === "admin" ? (
                              <div className="space-y-4">
                                ...
                                <button
                                  ...
                                  className="w-full font-black text-[10px] py-3.5 rounded-xl transition-all shadow-md uppercase tracking-widest border bg-white border-blue-200 text-blue-600 hover:bg-blue-50"
                                >
                                  Manage via Staff Panel
                                </button>
                              </div>
                            ) : (
                              <div className="space-y-4">
                                <p className="text-[10px] font-bold text-slate-400 leading-relaxed uppercase tracking-tight text-center">
                                  Standard user account without administrative
                                  access.
                                </p>
                                <button
                                  onClick={() => {
                                    setSelectedUser(null);
                                    setIsStaffDrawerOpen(true);
                                    setStaffSearchQuery(userDetails.user.email);
                                  }}
                                  className="w-full font-black text-[10px] py-3.5 rounded-xl transition-all shadow-md uppercase tracking-widest border bg-white border-slate-200 text-slate-600 hover:bg-slate-50"
                                >
                                  Manage Roles
                                </button>
                              </div>
                          ...
                          <div className="bg-red-50/20 p-6 rounded-2xl border border-red-100">
                            <div className="flex items-center gap-2 mb-4">
                              ...
                              <h3 className="font-bold text-red-900 uppercase tracking-wider text-[10px]">
                                Restricted Zone
                              </h3>
                            </div>
                            <button
                              ...
                              className={`w-full font-black text-[10px] py-3.5 rounded-xl transition-colors uppercase tracking-widest border ${
                                deleteLoading
                                  ? "bg-red-100 text-red-300 border-red-100 cursor-not-allowed"
                                  : "bg-white border-red-200 text-red-600 hover:bg-red-500 hover:text-white"
                              }`}
                            >
                              {deleteLoading
                                ? "Permanently Erasing..."
                        ...
                        {userDetails.orders?.length === 0 ? (
                          <div className="text-center py-20 bg-white rounded-2xl border border-slate-200 border-dashed">
                            <p className="text-[10px] font-black text-slate-300 uppercase tracking-widest">
                              No order interactions recorded
                            </p>
                          </div>
                        ) : (
                    ...
                    {activeTab === "cart" && (
                      <div>
                        {Object.keys(userDetails.user.cartData || {}).length ===
                        0 ? (
                          <div className="text-center py-20 bg-white rounded-2xl border border-slate-200 border-dashed">
                            <p className="text-[10px] font-black text-slate-300 uppercase tracking-widest">
                              Active cart is hollow
                            </p>
                          </div>
                        ) : (
                          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
                            {Object.entries(userDetails.user.cartData).map(
                              ([cartKey, cartValue]) => {
                                ...
                                if (cartKey.includes("::")) {
                                  ...
                                  if (
                                    ...
                                    totalQty = Object.values(cartValue).reduce(
                                      (acc, q) => acc + Number(q),
                                      0,
                                    );
                                    sizeArr = Object.entries(cartValue)
                                ...
                                let matchedVariant = code
                                  ? variants.find((v) => v.code === code)
                                  : null;
                                if (!matchedVariant)
                                  matchedVariant =
                                    variants.find(
                  ...
                  <div className="text-center p-20 bg-white rounded-2xl border border-red-100">
                    <p className="text-[10px] font-black text-red-400 uppercase tracking-widest">
                      Protocol Deviation: Failed to synchronize details
                    </p>
                  </div>
                )}
              </div>
              ...
              <div className="bg-slate-900 text-white p-6 shadow-lg z-10 flex justify-between items-center">
                <div>
                  ...
                  <div className="flex gap-2 mt-1">
                    <span className="bg-blue-600 text-white px-2 py-0.5 rounded text-[9px] font-black uppercase tracking-widest">
                        ...
                        >
                          <div className="flex gap-2 mb-4 overflow-x-auto no-scrollbar pb-1">
                            {(v.images || []).map((img, ii) => (
                              <img
                                key={ii}
                                src={img}
                                className="w-20 h-20 object-cover rounded-xl border border-slate-100 flex-shrink-0 shadow-sm"
                                alt=""
                              />
                            ))}
                          </div>
                          <div className="grid grid-cols-2 gap-4">
                            ...
                            <div className="col-span-2">
                              <p className="text-slate-400 text-[9px] font-black uppercase tracking-widest mb-2">
                              ...
                              <div className="flex gap-1.5 flex-wrap">
                                {(v.sizes || []).map((s) => (
                                  <span
                                    key={s}
                                    className="bg-slate-100 text-slate-600 px-2 py-0.5 rounded text-[9px] font-black border border-slate-200"
                                  >
                                    {s}
                                  </span>
                                ))}
            ...
            <div className="bg-white max-w-2xl w-full rounded-2xl shadow-2xl max-h-[92vh] overflow-hidden flex flex-col">
              ...
              <div className="bg-slate-900 text-white p-6 shadow-lg z-10 flex justify-between items-center">
                <div>
                  ...
                  <p className="text-slate-400 text-xs font-bold font-mono tracking-tighter">
                ...
                <div className="space-y-8">
                  ...
                  <div className="bg-white rounded-2xl p-6 border border-slate-200 shadow-sm grid grid-cols-2 gap-6 relative overflow-hidden">
                    ...
                    <div className="space-y-4">
                      ...
                      <div>
                        ...
                        <p className="font-bold text-slate-800 text-sm mt-0.5">
                          ...
                          {viewOrder.payment ? "(PAID)" : "(PENDING)"}
                        </p>
                      </div>
                    </div>
                    <div className="text-right flex flex-col justify-center">
                      ...
                      <p className="text-3xl font-black text-blue-600 tracking-tighter mt-1">
                        {currency}
                        {formatNumber(viewOrder.amount)}
                      </p>
                    </div>
                  </div>
                  ...
                  <div>
                    ...
                    <div className="space-y-4">
                      {viewOrder.items.map((item, idx) => (
                        <div
                          key={idx}
                          className="bg-white p-4 rounded-2xl border border-slate-200 shadow-sm flex gap-4 group"
                        >
                          <img
                            src={item.image}
                            className="w-20 h-20 object-cover rounded-xl shadow-sm border border-slate-100"
                          ...
                          <div className="flex-1 min-w-0">
                            ...
                            <div className="mt-2 flex gap-1 flex-wrap">
                              {item.size.map((s) => (
                                <span
                                  key={s}
                                  className="bg-blue-50 px-2 py-0.5 rounded text-[9px] font-black text-blue-600 border border-blue-100"
                                >
                                  {s}
                                </span>
                              ))}
              ...
              <div className="w-screen max-w-md">
                <div className="h-full flex flex-col bg-white shadow-2xl animate-in slide-in-from-right duration-300">
                    ...
                    <div className="flex items-center justify-between">
                      <div>
                        ...
                        <p className="text-slate-400 text-xs mt-1">
                          Configure administrative role access
                        </p>
                      </div>
                      <button
                        onClick={() => setIsStaffDrawerOpen(false)}
                    ...
                    <div className="mb-10">
                      ...
                      <div className="relative mb-4">
                        <input
                          ...
                          className="w-full pl-4 pr-10 py-3 bg-slate-50 border border-slate-200 rounded-xl focus:ring-4 focus:ring-blue-500/10 focus:border-blue-500 outline-none transition-all text-sm font-medium"
                        />
                        ...
                        <div className="space-y-2 max-h-48 overflow-y-auto pr-2 no-scrollbar">
                          {users
                            ...
                            .map((u) => (
                              <div
                                key={u._id}
                                className="flex items-center justify-between p-3 bg-white border border-slate-100 rounded-xl hover:border-blue-200 transition-all"
                              >
                                ...
                                <button
                                  ...
                                  className="ml-2 px-3 py-1.5 bg-blue-600 text-white rounded-lg text-[10px] font-bold hover:bg-blue-700 transition-all disabled:opacity-50"
                                >
                                  PROMOTE
                                </button>
                              </div>
                      ...
                      <div className="space-y-4">
                        {users
                          ...
                          .map((u) => (
                            <div
                              key={u._id}
                              className="p-4 bg-slate-50 border border-slate-200 rounded-2xl flex items-center gap-4 relative group hover:bg-white hover:border-blue-100 transition-all"
                            >
                              ...
                              <div className="flex-1 min-w-0">
                                <div className="flex items-center gap-2">
                                  ...
                                  {u._id === loggedInAdminId && (
                                    <span className="text-[8px] font-black bg-emerald-100 text-emerald-700 px-1.5 py-0.5 rounded tracking-tighter shadow-sm border border-emerald-200">
                                      YOU
                                    </span>
                                  )}
                                </div>
                              ...
                              {u._id !== loggedInAdminId && (
                                <button
                                  ...
                                  disabled={makeAdminLoading}
                                  className="opacity-0 group-hover:opacity-100 px-3 py-1.5 bg-red-50 text-red-600 border border-red-100 rounded-lg text-[9px] font-black transition-all hover:bg-red-600 hover:text-white"
                                >
                                  REVOKE
                                </button>
                              )}
                  ...
                  <div className="p-6 bg-slate-50 border-t border-slate-100">
                    <p className="text-[10px] font-bold text-slate-400 leading-relaxed uppercase tracking-tight text-center">
                      Granting admin access allows the user to view orders,
                      manage products, and modify system settings.
                    </p>
                  </div>
                </div>
              </div>
