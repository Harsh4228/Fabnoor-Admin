    ...
    <div className="min-h-screen bg-gradient-to-br from-gray-50 to-gray-100 py-8 px-4 md:px-10">
      <div className="max-w-7xl mx-auto">
        ...
        <div className="mb-8 text-center">
          <h2 className="text-3xl md:text-4xl font-bold text-gray-900 mb-2">Admin Orders</h2>
        ...
        {paginatedOrders.length === 0 ? (
          <div className="bg-white rounded-3xl shadow-lg p-12 text-center">
            ...
            <p className="text-gray-400 mt-2">Orders with "{activeStatus}" status will appear here</p>
          </div>
        ) : (
            ...
            <div className="bg-white w-full max-w-5xl rounded-3xl shadow-2xl max-h-[95vh] overflow-y-auto">
              ...
              <div className="sticky top-0 bg-gradient-to-r from-blue-600 to-purple-600 text-white p-6 rounded-t-3xl z-10 shadow-lg">
                <div className="flex justify-between items-center">
                  <div>
                    ...
                    <p className="text-sm text-blue-100 mt-1">Order ID: {selectedOrder._id}</p>
                  </div>
                  <button
              ...
              <div className="p-6 space-y-6">
                ...
                <div className="bg-gradient-to-br from-blue-50 to-purple-50 p-6 rounded-2xl border border-blue-100">
                  <h4 className="font-bold text-lg mb-4 text-gray-800">Order Summary</h4>
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4 text-sm">
                    <div className="flex justify-between">
                      <span className="text-gray-600">Date:</span>
                      <span className="font-semibold">{new Date(selectedOrder.date).toLocaleString()}</span>
                    </div>
                    <div className="flex justify-between">
                      <span className="text-gray-600">Status:</span>
                      <span className={`font-semibold ${statusColor(selectedOrder.status)}`}>{selectedOrder.status}</span>
                    </div>
                    <div className="flex justify-between">
                      <span className="text-gray-600">Payment:</span>
                      <span className={`font-semibold ${selectedOrder.payment ? "text-green-600" : "text-red-600"}`}>
                    ...
                    <div className="flex justify-between md:col-span-2">
                      ...
                      <span className="font-bold text-xl text-blue-600">{currency}{selectedOrder.amount}</span>
                    </div>
                  </div>
                </div>
                ...
                <div className="bg-green-50 p-6 rounded-2xl border border-green-100">
                  <h4 className="font-bold text-lg mb-3 text-gray-800">Customer Information</h4>
                  <div className="space-y-2 text-sm">
                    <p><span className="font-semibold text-gray-700">Name:</span> {selectedOrder.address.fullName}</p>
                    <p><span className="font-semibold text-gray-700">Phone:</span> {selectedOrder.address.phone}</p>
                  </div>
                </div>

                {/* ADDRESS */}
                <div className="bg-orange-50 p-6 rounded-2xl border border-orange-100">
                  <h4 className="font-bold text-lg mb-3 text-gray-800">Shipping Address</h4>
                  <div className="text-sm space-y-1 text-gray-700">
                    <p>{selectedOrder.address.street}</p>
                    <p>{selectedOrder.address.city}, {selectedOrder.address.state}</p>
                    <p>{selectedOrder.address.country} - {selectedOrder.address.pinCode}</p>
                  </div>
                </div>
                ...
                <div className="bg-gray-50 p-6 rounded-2xl border border-gray-200">
                  ...
                  >
                    <svg className={`w-5 h-5 transition-transform ${showRaw ? "rotate-90" : ""}`} fill="none" stroke="currentColor" viewBox="0 0 24 24">
