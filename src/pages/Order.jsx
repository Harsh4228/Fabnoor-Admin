...
const Order = ({ token }) => {
  ...
  const handleDeleteOrder = async () => {
    ...
    try {
      ...
      if (res.data.success) {
        ...
        if (selectedOrder?._id === deleteModal.orderId) setSelectedOrder(null);
  ...
  /* ================= DOWNLOAD SHIPPING SLIP (WAYBILL) ================= */  const downloadInvoice = async (orderId) => {
    try {
                  ...
                  >
                    ...
                    <div className="grid grid-cols-2 gap-2">
                      ...
                      <button
                        onClick={(e) => {
                          e.stopPropagation();
                          setDeleteModal({ show: true, orderId: order._id, orderNumber: order.orderNumber || order._id });
                        }}
                        className="col-span-2 py-2 rounded-lg text-[9px] font-black uppercase tracking-widest border border-red-200 text-red-500 bg-red-50 hover:bg-red-100 hover:border-red-400 active:scale-95 transition-all flex items-center justify-center gap-1.5"
                      >
      ...
      {deleteModal.show && (
        <div className="fixed inset-0 bg-slate-900/60 backdrop-blur-sm flex items-center justify-center p-4 z-[60] overflow-hidden">
          <div className="bg-white rounded-[2rem] shadow-2xl p-8 w-full max-w-sm text-center">
            ...
            <h3 className="text-xl font-black text-slate-900 tracking-tight">Delete Permanently?</h3>
            <p className="text-sm font-medium text-slate-500 mt-2">
              Order <span className="font-black text-slate-800">{deleteModal.orderNumber}</span> will be <span className="text-red-600 font-bold">permanently removed</span> from the system. This cannot be undone.
            </p>
            <div className="mt-8 flex gap-3">
              <button
                onClick={() => setDeleteModal({ show: false, orderId: null, orderNumber: "" })}
                disabled={deleteLoading}
                className="flex-1 bg-slate-100 text-slate-700 py-3.5 rounded-xl font-black uppercase tracking-widest text-[10px] hover:bg-slate-200 transition-colors disabled:opacity-50"
              >
                Cancel
              </button>
              <button
                ...
                disabled={deleteLoading}
                className="flex-1 bg-red-600 text-white py-3.5 rounded-xl font-black uppercase tracking-widest text-[10px] hover:bg-red-700 shadow-lg shadow-red-200 transition-all active:scale-95 disabled:opacity-60 flex items-center justify-center gap-2"
              >
                {deleteLoading ? <><div className="w-3.5 h-3.5 border-2 border-white/30 border-t-white rounded-full animate-spin" />Deleting...</> : "Yes, Delete"}
              </button>
            </div>
          </div>
        </div>
